...
{
    /// <summary>
    /// This class contains all generic methods which can be re-used.
    /// </summary>
    public static class GaiaUtils
    {
        // The number of bytes contained in a int.
        private const int BYTES_IN_INT = 4;
        // The number of bits contained in a byte.
        private const int BITS_IN_BYTE = 8;

        /// <summary>
        /// This method allows retrieval of a human readable representation of an hexadecimal value contained in a int.
        /// </summary>
        ...
        /// <returns>String object containing values in byte array formatted as hex.</returns>
        ...
        /// <summary>
        /// Extract an int value from a bytes array.
        /// </summary>
        /// <param name="source">The array to extract from.</param>
        /// <param name="offset">Offset within source array.</param>
        /// <param name="length">Number of bytes to use (maximum 4).</param>
        /// <param name="reverse">True if bytes should be interpreted in reverse (little endian) order.</param>
        /// <returns>The extracted int</returns>
        public static int ExtractIntFromByteArray(byte[] source, int offset, int length, bool reverse)
        {
            if (length < 0 | length > BYTES_IN_INT)
            {
                throw new IndexOutOfRangeException("Length must be between 0 and " + BYTES_IN_INT);
            }
        ...
        /// <summary>
        /// This method allows copy of an int value into a byte array from the specified offset location to the offset + length location.
        /// </summary>
        /// <param name="sourceValue">The int value to copy in the array.</param>
        /// <param name="target">The byte array to copy in the int value.</param>
        /// <param name="targetOffset">The targeted offset in the array to copy the first byte of the int value.</param>
        /// <param name="length">The number of bytes in the array to copy the int value.</param>
        /// <param name="reverse">True if bytes should be interpreted in reverse (little endian) order.</param>
        public static void CopyIntIntoByteArray(int sourceValue, byte[] target, int targetOffset, int length, bool reverse)
        {
            if (length < 0 | length > BYTES_IN_INT)
            {
            ...
            {
                throw new IndexOutOfRangeException("The targeted location must be contained in the target array.");
            }
        ...
        /// <summary>
        /// To get a String label which corresponds to the given GAIA command.
        /// The label is built as follows:
        /// The value of the GAIA command as an hexadecimal given by GetHexadecimalStringFromInt
        /// The name of the GAIA command as defined in the protocol or UNKNOWN if the value cannot be matched with the known ones.
        /// Optional: "(deprecated)" is the command had been deprecated.
        /// For instance, for the given value 384 the method will return "0x0180 COMMAND_GET_CONFIGURATION_VERSION"
        /// </summary>
        /// <param name="command">The command to obtain a label for.</param>
        /// <returns>the label corresponding to the given command.</returns>
        public static string GetCommandToString(int command)
        {
            string name = "UNKNOWN";
